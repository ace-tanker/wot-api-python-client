...
class GlobalmapEventaccountinfoErrorErrorOneOf(BaseModel):
    """
    ...
    def code_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set([402]):
            raise ValueError("must be one of enum values (402)")
        return value

    @field_validator('message')
    def message_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['ACCOUNT_ID_OR_CLAN_ID_NOT_SPECIFIED']):
            raise ValueError("must be one of enum values ('ACCOUNT_ID_OR_CLAN_ID_NOT_SPECIFIED')")
        return value
