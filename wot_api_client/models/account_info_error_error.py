...
class AccountInfoErrorError(BaseModel):
    """
    ...
    def code_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set([407]):
            raise ValueError("must be one of enum values (407)")
        return value

    @field_validator('message')
    def message_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['ACCOUNT_ID_LIST_LIMIT_EXCEEDED']):
            raise ValueError("must be one of enum values ('ACCOUNT_ID_LIST_LIMIT_EXCEEDED')")
        return value
