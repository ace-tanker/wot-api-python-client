...
class AuthLoginErrorErrorOneOf2(BaseModel):
    """
    ...
    def code_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set([410]):
            raise ValueError("must be one of enum values (410)")
        return value

    @field_validator('message')
    def message_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['AUTH_ERROR']):
            raise ValueError("must be one of enum values ('AUTH_ERROR')")
        return value
